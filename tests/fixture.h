...
source_snapshot(std::string version)
{
  ...
  return seal_source(
      ...
      recipe_declaration(
          ...
          {
            lifecycle_program(lifecycle_action::pre_install,
              program(program_language::posix_shell, "echo pre-install\n")),
            lifecycle_program(lifecycle_action::post_install,
              program(program_language::posix_shell, "echo post-install\n")),
            lifecycle_program(lifecycle_action::pre_remove,
              program(program_language::posix_shell, "echo pre-remove\n")),
            lifecycle_program(lifecycle_action::post_remove,
              program(program_language::posix_shell, "echo post-remove\n")),
          },
...
historical_control(
    std::string format = "text/x-posix-shell")
{
  return pkgplan::installed_control_projection(
      ...
      {pkgplan::runtime_dependency_declaration::make("libc >= 0")},
      {
        pkgplan::removal_lifecycle_declaration::make(
          ...
          format, "echo old-pre-remove\n"),
        pkgplan::removal_lifecycle_declaration::make(
          pkgplan::removal_lifecycle_phase::post_remove,
          format, "echo old-post-remove\n"),
      },
...
installed(const planning_authorities& authorities,
          ...
          pkgplan::installed_control_projection control = historical_control())
{
...
[[nodiscard]] inline pkgplan::removal_plan
removal_with_control(const planning_authorities& authorities,
                     pkgplan::installed_control_projection control,
                     std::string path = "tool")
{
  const auto logical = pkgplan::package_path::parse(path);
  const auto active = regular_object(1, 0755);
  pkgplan::removal_request request(
      installed(authorities, release(1, "1.0"), std::move(control)),
      authorities.snapshot,
      ownership(authorities, {pkgplan::installed_ownership_claim(
          logical, authorities.installed_package, active)}),
      authorities.target,
      observations(authorities, {pkgplan::target_path_observation::present(
          pkgplan::filesystem_object_fact(logical, active))}),
      policy_snapshot(authorities));
  pkgplan::removal_result result = pkgplan::plan_removal(request);
  if (!result.has_plan() || result.plan() == nullptr)
    throw std::runtime_error("custom removal fixture was refused");
  return *result.plan();
}
