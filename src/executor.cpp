...
prepared_execution prepare(const admitted_lifecycle_session& session) {
  ...
  const auto target_identity=resource_identity("pkgapply-exec/managed-target-root/v1",session.node().identity().hex());
  const auto temporary_slot=pkgexec::resource_slot::singleton(pkgexec::resource_role::private_temporary_root);
  const auto temporary_identity=resource_identity("pkgapply-exec/private-temporary-root/v1",session.node().identity().hex());
